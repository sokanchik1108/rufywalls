...
#finishOrder,#addBatchToOrder,#openProductModal { background:#3b82f6; color:#fff; border:none; font-weight:600; cursor:pointer; text-transform:uppercase; transition:all 0.2s; }
...
.order-item { background:#fff; padding:12px; margin-bottom:12px; box-shadow:0 2px 6px rgba(0,0,0,0.05); position:relative; border-radius:10px; }
...
select.form-select { color:#4f46e5; font-weight:600; }
...
.modal-fullscreen .modal-header { border-bottom:1px solid #e5e7eb; padding-bottom:8px; }
.modal-fullscreen .modal-title { font-size:18px; font-weight:600; color:#333; }
.modal-body { flex:1; overflow-y:auto; margin-top:8px; }
.batch-card { background:#f9f9fb; border-radius:12px; padding:14px; margin-bottom:12px; cursor:pointer; display:flex; justify-content:space-between; align-items:center; font-size:16px; box-shadow:0 2px 6px rgba(0,0,0,0.03); transition:all 0.2s; }
.batch-card:hover { background:#f3f4f6; transform:translateY(-2px); }
.qty-input { width:80px; font-size:16px; padding:6px 8px; border-radius:10px; border:1px solid #ddd; font-weight:600; }
#orderTotal { font-weight:700; font-size:18px; margin-top:12px; text-align:right; color:#2563eb; }
.discount-label { font-weight:600; margin-bottom:4px; display:block; }
</style>
...
<div class="container">
    <form method="POST" action="{{ route('admin.orders.store') }}" id="orderForm">
        ...
        <!-- Надпись для скидки -->
        <label for="discountInput" class="discount-label">Скидка (тг)</label>
        <input type="number" id="discountInput" name="discount" class="form-control mb-2" placeholder="Введите сумму скидки" value="0" min="0">
...
<script>
let itemIndex=0,currentBatch={},batchWarehouses=[],batchesState={};

function resetProductModal() { $('#modalSkuInput').val(''); $('#modalBatchesSimple').empty(); }
function resetBatchDetailModal() { $('#batchDetailTable tbody').empty(); currentBatch={}; batchWarehouses=[]; }
...
function loadBatchesSimple(sku){
    ...
    $.get('/admin/batches/by-sku/'+sku,function(data){
        ...
        data.forEach(batch=>{
            if(!batchesState[batch.id]){ batchesState[batch.id]={}; batch.warehouses.forEach(w=>batchesState[batch.id][w.id]=w.pivot.quantity); }
...
$('#addBatchToOrder').click(function(){
    ...
    $('#batchDetailTable tbody tr').each(function(){
        ...
        if(qty>available){ alert(`Недостаточно товара на складе ${warehouseId}.`); return; }
        const warehouseName=currentBatch.warehouses.find(w=>w.id==warehouseId).name;
        $('#itemsWrapper').append(`<div class="order-item" data-batch-id="${currentBatch.id}" data-warehouse-id="${warehouseId}">
            ...
            <div class="order-item-bottom">
                <span>${qty} × <input type="number" name="items[${itemIndex}][price]" class="price-input" step="0.01" placeholder="цена" required></span>
...
$(document).on('input','.price-input',function(){ recalcOrderTotal(); });
$(document).on('input','#discountInput',function(){ recalcOrderTotal(); });

$('#orderForm').submit(function(e){
    let discountVal = parseFloat($('#discountInput').val());
    if(isNaN(discountVal)||discountVal<0) $('#discountInput').val(0);

    let invalid=false;
    $('.price-input:visible').each(function(){
        if(!$(this).val()||parseFloat($(this).val())<=0){ invalid=true; $(this).addClass('is-invalid'); } else { $(this).removeClass('is-invalid'); }
    });
    if(invalid){ alert('Пожалуйста, укажите цену для всех товаров перед созданием заказа.'); e.preventDefault(); }
});
