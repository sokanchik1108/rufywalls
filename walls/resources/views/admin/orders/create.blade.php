...
.order-item-top { display: flex; justify-content: space-between; font-weight: 600; align-items: center; }
.warehouse { color: #4f46e5; margin-right: 10px; font-weight: 500; }
.order-item-bottom { margin-top: 6px; display: flex; flex-direction: column; gap: 4px; }
.order-item-bottom span, .available-total, .batch-warehouses, td:nth-child(2), td:nth-child(3) { font-weight: 600; }
...
.available-total { font-size: 14px; color: #666; margin-top: 4px; font-weight: 600; }
...
$('#addBatchToOrder').click(function() {
    ...
    $('#batchDetailTable tbody tr').each(function() {
        ...
        if(!qty || qty <= 0) { alert('Введите количество!'); hasError = true; return false; }
        const warehouseId = $input.data('warehouse');
        let available = batchesState[currentBatch.id][warehouseId] || 0;
        if(qty > available) { alert(`Недостаточно товара на складе ${warehouseId}.`); hasError = true; return false; }
...
$('#orderForm').submit(function(e) {
    ...
    $('.price-input:visible').each(function() {
        if(!$(this).val() || parseFloat($(this).val()) <= 0) { invalid = true; $(this).addClass('is-invalid'); }
